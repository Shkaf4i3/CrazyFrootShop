...
async def check_balance_id(tg_id):
    connection = await db_pool.connection()
    cursor = connection.transaction()
    await cursor.begin()
    ...
    if balance >= 250:
        await cursor.execute(querystring='UPDATE users SET balance = balance - 250 WHERE id = ($1)',
                             parameters=[BigInt(tg_id)])
        await cursor.commit()
        return True
    else:
...
async def app_balance(amount, tg_id):
    ...
    await cursor.execute(querystring='UPDATE users SET balance = ($1) - 250 WHERE id = ($2)',
